    ...
    <div className="space-y-3 rounded-xl border border-line bg-surface-2 p-4">
      ...
      <Section title="Connection">
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2"><select value={settings.model} onChange={(e) => patch({ model: e.target.value })} className="w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-cream focus:border-line-2 focus:outline-none"><option value="auto">auto / let OmniRoute choose</option>{models.map((model) => <option key={model} value={model}>{model}</option>)}</select><Btn variant="ghost" onClick={loadModels} title="Load models"><RefreshCw size={15} /></Btn></div>
